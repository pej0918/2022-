...
def home():
    ...
    for i in range(1, 5):#125):
        for j in range(6):
            clicks = '//*[@id="choiceTxt"]/div/div/a[{}]'.format(i)
            driver.find_element_by_xpath(clicks).send_keys(Keys.ENTER)
...
def web_forest_post():
    emd_receive=request.form['emd_give']
    gun_receive = request.form['gun_give']

    doc={
        'emd':emd_receive,
        'gun':gun_receive
    }
    db.forestnumbersearch.drop()
    db.forestnumbersearch.insert_one(doc)
    return jsonify({'msg':'검색 완료!'})

@app.route('/forests', methods=['GET'])
def forestnumbersearch():
    final_list=[]
    gan_emd=list(db.forestnumbersearch.find({},{'_id':False}))[0]['emd']
    gan_gungu=list(db.forestnumbersearch.find({},{'_id':False}))[0]['gun']
    forest_maxi=db.forests.find_one({'emd': gan_emd, 'gungu': gan_gungu})['maxi']

    final_list=[forest_maxi, gan_emd, gan_gungu]

    return jsonify({'forestsearch':final_list})


@app.route('/forestfire_01.html')
def forest1():
    click_forest = list(db.forestname.find({}, {'_id': False}))[0]['san_name']
    return render_template('forestfire_01.html',temptext5=click_forest)
